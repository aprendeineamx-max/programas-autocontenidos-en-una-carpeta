...
Este repo agrupa aplicaciones Windows en modo “portable” (binarios, datos y configuraciones dentro de la misma carpeta), para moverlas a otra PC o una USB. Incluye scripts para rehidratar (reinstalar servicios/drivers cuando aplica), lanzar apps con APPDATA/LOCALAPPDATA/PROGRAMDATA/TEMP redirigidos al repo y hacer diagnósticos/bkps.
...
- Tokens DPAPI no son portables entre equipos: en una nueva PC tendrás que iniciar sesión una vez en cada app que use autenticación vinculada al equipo/usuario.

## Estructura
```
apps/                 Binarios por app (ej. apps/ProtonVPN/bin)
data/                 Datos/config por app (redirige APPDATA/LOCALAPPDATA/PROGRAMDATA/USERPROFILE/TEMP)
...
1) (Opcional) Habilitar rutas largas si al descomprimir da error de “Path too long”: doble clic `enable_long_paths.bat` y reinicia sesión/PC.
